...
Скачивайте **браузером со страницы релизов**. Если получить DMG через мессенджер
(Telegram, WhatsApp) или распаковать приложением из песочницы, macOS пометит файл
как созданный песочницей и откажется запускать — `File created by an AppSandbox`.
Лечится снятием расширенных атрибутов:
